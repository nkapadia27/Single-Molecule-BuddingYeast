truncp=min(On_time_final);
pdf_exp_two_kinetics = @(data,p,Tboundalpha,Tboundbeta,Tbleach) p*(((Tbleach + Tboundalpha)/(Tboundalpha*Tbleach))*exp(-(((Tbleach + Tboundalpha)/(Tboundalpha*Tbleach))*(data-truncp))))+...
    (1-p)*(((Tbleach + Tboundbeta)/(Tboundbeta*Tbleach))*exp(-(((Tbleach + Tboundbeta)/(Tboundbeta*Tbleach))*(data-truncp))))

time_int=1
t1=[min(On_time_final):time_int:max(On_time_final)];
pstart=0.5;
...
Tbleach_lb=Tbleachstart*0.8;
Tbleach_ub=Tbleachstart*1.2;
...
options = optimoptions('fmincon','Algorithm','interior-point',...
    'Display','off',...
    'GradObj','on','MaxIter',2000);
x=[0:0.1:max(On_time_final)];
 num_of_bins=ceil(sqrt(numel(On_time_final)));
...
%uMLE=mle(On_time_final, 'pdf',pdf_exp_two_kinetics, 'start',start,'LowerBound',lb,'UpperBound',ub,'options',options2,'OptimFun','fmincon')
 [uMLE,unLogL] = fmincon(@(on) double_nloglike_exp_withgrad ([on(1) on(2) on(3) on(4)],On_time_final),start,[],[],[],[],lb,ub,[],options);
...
% use = sqrt(diag(uacov))
figure,histogram(On_time_final,length(t1),'Normalization','pdf')
hold on
...
[hratio,pValue_ratio,stat_ratio,cValue_ratio] = lratiotest(uLogL,cLogL,1,0.005)

[aic bic]=aicbic([cLogL,uLogL], ...
    [2,4],length(On_time_final))
